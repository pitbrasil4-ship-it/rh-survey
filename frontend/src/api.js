...
export const api = {
  ...
  surveys: {
    ...
    duplicate:  (id, name) => request('POST', `/surveys/${id}/duplicate`, name ? { name } : {}),
    export:     (id)       => request('GET', `/surveys/${id}/export`),
    versions:   (id)       => request('GET', `/surveys/${id}/versions`),
    history:    (id)       => request('GET', `/surveys/${id}/history`),
    publish:    (id, note) => request('POST', `/surveys/${id}/publish`, note ? { note } : {}),
  ...
  dimensions: {
    ...
    create:       (data)      => request('POST', '/dimensions', data),
    update:       (id, data)  => request('PUT', `/dimensions/${id}`, data),
    remove:       (id)        => request('DELETE', `/dimensions/${id}`),
  },
  invitations: {
    list:      (surveyId)        => request('GET',  `/invitations/survey/${surveyId}`),
    adherence: (surveyId)        => request('GET',  `/invitations/survey/${surveyId}/adherence`),
    create:    (surveyId, data)  => request('POST', `/invitations/survey/${surveyId}`, data),
    send:      (surveyId)        => request('POST', `/invitations/survey/${surveyId}/send`),
    remind:    (surveyId)        => request('POST', `/invitations/survey/${surveyId}/remind`),
    schedule:  (surveyId, runAt) => request('POST', `/invitations/survey/${surveyId}/schedule`, { runAt }),
    unschedule:(id)              => request('DELETE', `/invitations/schedule/${id}`),
    remove:    (id)              => request('DELETE', `/invitations/${id}`),
  },
  campaigns: {
    list:   ()         => request('GET', '/campaigns'),
    create: (data)     => request('POST', '/campaigns', data),
    update: (id, data) => request('PUT', `/campaigns/${id}`, data),
    remove: (id)       => request('DELETE', `/campaigns/${id}`),
  },
  respondents: {
